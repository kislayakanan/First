    ...
    deinit {
        ...
        for infoDict in textFieldInfoCache {
            ...
            if let textField = view as? UITextField {
                ...
                let returnKeyTypeValue = infoDict.objectForKey(kIQTextField) as! NSNumber
                textField.returnKeyType = UIReturnKeyType(rawValue: returnKeyTypeValue.integerValue)!
                
                textField.delegate = infoDict.objectForKey(kIQTextField) as! UITextFieldDelegate?
            } else if let textView = view as? UITextView {
                
                textView.returnKeyType = UIReturnKeyType(rawValue: (infoDict.objectForKey(kIQTextField) as! NSNumber).integerValue)!
                
                let returnKeyTypeValue = infoDict.objectForKey(kIQTextField) as! NSNumber
                textView.returnKeyType = UIReturnKeyType(rawValue: returnKeyTypeValue.integerValue)!
                
                textView.delegate = infoDict.objectForKey(kIQTextField) as! UITextViewDelegate?
            }
        }
